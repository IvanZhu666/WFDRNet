...
class PatchEmbedding(nn.Module):
    ...
    def forward(self, x):
        patches = self.patch_embeddings(x)  # (B, embed_dim, num_patches ** 2 / 1)
        patches = patches.flatten(2)  # (B, embed_dim, num_patches ** 2)
        patches = patches.transpose(1, 2)  # (B, num_patches ** 2, embed_dim)
        return patches


class PositionalEncoding(nn.Module):
    def __init__(self, embed_dim, max_len=1024):  # For 224x224 with 16x16 patches
...
class FDB(nn.Module):
    ...
    def forward(self, x):
        ...
        for layer in self.encoder_layers:
            x = layer(x)
        x = self.norm(x)
        x = self.adaptive_pool0(x)# (B, 1024, 768) -> (B, 1024, 512)

        x = x.reshape(x.size(0), -1, 32, 32)  # (B, 1024, 512) -> (B, 512, 32, 32)
        x4 = self.conv1(x)  # (B, 512, 32, 32) -> (B, 1024, 8, 8)
        x3 = self.upconv1(x4)  # (B, 1024, 8, 8) -> (B, 512, 16, 16)
        x2 = self.upconv2(x3)  # (B, 512, 16, 16) -> (B, 256, 32, 32)
        x1 = self.upconv3(x2)  # (B, 256, 32, 32) -> (B, 128, 64, 64)

        return (x1, x2, x3, x4)

if __name__ == "__main__":
    # 定义图像转换
    student_net = FDB().cuda()
    input_tensor = torch.randn(32, 384, 64, 64)
    input_tensor = input_tensor.cuda()
    with torch.no_grad():
        output = student_net(input_tensor)
